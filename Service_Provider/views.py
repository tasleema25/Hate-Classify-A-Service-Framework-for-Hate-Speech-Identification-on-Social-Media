...
def View_TweetClassify_Type_Ratio(request):
    ...
    # Fetch all instances
    instances = hate_classify_prediction.objects.all()
    ...
    # Classify instances and count predictions
    for instance in instances:
        if instance.Prediction == 'Hate Speech':
            hate_count += 1
        elif instance.Prediction == 'Offensive Speech':
            offensive_count += 1
        elif instance.Prediction == 'Non Offensive Speech':
    ...
    ratio_hate = (hate_count / total_count) * 100
    ratio_offensive = (offensive_count / total_count) * 100
    ratio_non_offensive = (non_offensive_count / total_count) * 100

    # Create objects in the detection_ratio model
    if ratio_hate != 0:
        detection_ratio.objects.create(names='Hate Speech', ratio=ratio_hate)
    if ratio_offensive != 0:
        detection_ratio.objects.create(names='Offensive Speech', ratio=ratio_offensive)
    if ratio_non_offensive != 0:
        detection_ratio.objects.create(names='Non Offensive Speech', ratio=ratio_non_offensive)
